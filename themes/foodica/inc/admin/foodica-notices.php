...
class Foodica_Notices {
	...
	public function hide_notices() {
		...
		if ( isset( $_GET['foodica-hide-notice'] ) && isset( $_GET['_foodica_notice_nonce'] ) ) {
			...
			if ( ! wp_verify_nonce( wp_unslash( $_GET['_foodica_notice_nonce'] ), 'foodica_hide_notices_nonce' ) ) {
				wp_die( __( 'Action failed. Please refresh the page and retry.', 'foodica' ) );
			}

			if ( ! current_user_can( 'manage_options' ) ) {
				wp_die( __( 'Cheatin&#8217; huh?', 'foodica' ) );
			}

			if ( $_GET['foodica-hide-notice'] ) {
